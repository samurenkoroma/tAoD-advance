...
func (d *db) Create(ctx context.Context, user user.User) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to create user due to error %v", user)
	}
...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if result.Err() != nil {
		if errors.Is(result.Err(), mongo.ErrNoDocuments) {
			return u, fmt.Errorf("ErrrEntityNotFound")
		}
...
func (d *db) FindAll(ctx context.Context) (u []user.User, err error) {
	...
	if cursor.Err() != nil {
		return u, fmt.Errorf("failed to find all user due to err: %v", err)
	}

	if err := cursor.All(ctx, &u); err != nil {
		return u, fmt.Errorf("failed to read all documents from cursor due to err %v", err)
	}
